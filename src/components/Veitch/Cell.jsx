    ...
    elements = border.styles.map((cell, index) => {
      ...
      for (const key in cell) {
        if (cell.hasOwnProperty(key) && typeof cell[key] === 'boolean') {
          if (cell[key]) {
            stylesCell['border' + key] = '2px solid ' + cell.color
          }
        }
      }
      if (cell.Top) {
        stylesCell.height = `calc(100% - ${cell.index * 2}px)`
        stylesCell.top = `${cell.index * 2}px`
      }
      if (cell.Bottom) {
        stylesCell.height = `calc(100% - ${cell.index * 2}px)`
        stylesCell.bottom = `${cell.index * 2}px`
        stylesCell.top = `auto`
      }
      if (cell.Left) {
        stylesCell.width = `calc(100% - ${cell.index * 2}px)`
        stylesCell.left = `${cell.index * 2}px`
      }
      if (cell.Right) {
        stylesCell.width = `calc(100% - ${cell.index * 2}px)`
      ...
      if (cell.Top && cell.Bottom) {
        stylesCell.height = `calc(100% - ${cell.index * 4}px)`
        stylesCell.top = `${cell.index * 2}px`
        stylesCell.bottom = `auto`
      }
      if (cell.Left && cell.Right) {
        stylesCell.width = `calc(100% - ${cell.index * 4}px)`
        stylesCell.left = `${cell.index * 2}px`
