...
function VeitchApp() {
  ...
  const changeButtonHandler = (id) => {
    ...
    let newCombinations = newCellDate
      .reduce((acum, button, index) => {
        ...
        const cellArrayCheckerWrapper = ({ statement, elements }) => {
          if (statement) {
        ...
        const cellCheckerWrapper = ({ statement, functionName, cellArr }) => {
          if (statement) {
        ...
        const cellDataArray = [
          {
            statement: newCellDate.length > index + 7 && button.id % SIZE === 0,
            functionName: cellChecker,
            cellArr: [{ start: index, end: index + 7 }],
          },
          {
            statement: index < SIZE - 1,
          ...
          {
            statement: newCellDate.length > index + 3 && button.id % SIZE === 0,
            functionName: cellChecker,
            cellArr: [{ start: index, end: index + 3 }],
          },
          {
            statement: index < SIZE * (SIZE - 1) && (index + 1) % SIZE === 0,
            functionName: cellCheckerVertically,
            cellArr: [
              { start: index, end: 2 },
          ...
          {
            statement:
              index < SIZE * (SIZE - 1) - 1 && (index + 1) % SIZE !== 0,
            functionName: cellCheckerVertically,
            cellArr: [
              { start: index, end: 2 },
              { start: index + 1, end: 2 },
        ...
        const cellArrayCheckerArray = [
          {
            statement: index === newCellDate.length - 1,
  ...
  return (
    <div className={styles.app}>
      <h1 className={styles.h1}>Розрахунок МДНФ за допомогою таблиці Вейча</h1>
